...
const HomeAdmin = () => {
  ...
  return (
    <div className="flex h-screen">
      {/* MENU LATERAL */}
      <div className={`fixed top-0 left-0 h-full bg-[#043f61] transition-all duration-300 flex flex-col justify-between ${isSidebarOpen ? 'w-52' : 'w-14'}`}>
        <div>
          <div className={`${isSidebarOpen ? 'py-3 px-8' : 'p-3'}`}>
            <img src={isSidebarOpen ? logoG : logoP} alt="Logo-Ost" className='w-full h-auto' />
          </div>
          <nav className="mt-2">
            <NavItem icon={faHouse} name="Inicio" link="#" isOpen={isSidebarOpen} />
            <NavItem icon={faShop} name="Ventas" link="#" isOpen={isSidebarOpen} />
            <NavItem icon={faUsers} name="Clientes" link="#" isOpen={isSidebarOpen} />
          </nav>
        </div>
